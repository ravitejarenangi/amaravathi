...
class User_model extends CI_Model
{
    ...
    {
        $this->db->select('users.*,students.firstname,students.image,students.middlename,students.lastname,students.guardian_name,classes.id AS `class_id`,classes.class,sections.id AS `section_id`,sections.section,students.admission_no,students.mobileno,students.email');
        $this->db->from('users');
    ...
    public function checkStudentStatus($id, $user_type) {
        $this->db->select('students.is_active,students.id');
        ...
        if ($query1->is_active != 'no') {
            ...
            if($user_type == 'student'){
                $status = $this->getUserLoginDetails($query1->id);
                return $status['is_active'];
                 
            }elseif($user_type == 'parent'){
                $status = $this->getParentLoginDetails($query1->id);
                return $status['is_active'];
            }
