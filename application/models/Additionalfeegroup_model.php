...
class Additionalfeegroup_model extends MY_Model
{
    ...
    {
        $this->db->select()->from('additionalfee_groups');
    ...
    public function remove($id)
    {
        ...
        $this->db->where('id', $id);
        $this->db->where('is_system', 0);
        $this->db->delete('additionalfee_groups');
    ...
    public function add($data)
    {
        ...
        if (isset($data['id'])) {
            ...
            $this->db->update('additionalfee_groups', $data);
            $message   = UPDATE_RECORD_CONSTANT . " On  fee group id " . $data['id'];
            $action    = "Update";
            $record_id = $id = $data['id'];
            $this->log($message, $record_id, $action);
        } else {
            $this->db->insert('additionalfee_groups', $data);
    ...
    public function check_data_exists($name, $id)
    {
        ...
        $this->db->where('id !=', $id);
        $query = $this->db->get('additionalfee_groups');
        if ($query->num_rows() > 0) {
            return true;
        } else {
            return false;
